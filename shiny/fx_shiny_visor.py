...
app_ui = ui.page_fluid(
    ...
    ui.layout_sidebar(
        ui.sidebar(
            ...
            ui.input_switch("lock_dates", "Lock Date Window", value=True),
            ui.h4("FX-Portfolio Diversification"),
            ui.input_radio_buttons(
                id="blending_type",
                label="Type of Portfolio blending:",
                choices={"ew": "Equally Weighted", "iv": "Inverse-Volatility Weighted"},
                selected="iv",
                inline=True,
            ),
            ...
            ui.nav_menu(
                ...
                ui.nav_panel(
                    ...
                    ui.row(
                        ...
                        ui.column(
                            4, ui.h3("Distribution"), ui.output_table("alloc_div")
                        ),
                    ),
                    ui.row(
                        ui.column(
...
def server(input, output, session):
    ...
    def fetch_and_clean():
        ...
        if len(input.symbols()) == 1:
            ...
            df.columns = [
                x.replace("EUR", "").replace("=X", "") for x in df.columns.tolist()
            ]
        return df
    ...
    def ew_port_cumret():
        return (
            10000 * (1 + normalized_px().pct_change().fillna(0).mean(axis=1)).cumprod()
        )
    ...
    def iv_factor_weigths():
        ...
        factors[factors > 1] = (
            1  # DKK patology (pegged to EUR). It acts as a risk-free currency ~EUR
        )
        return factors

    @reactive.Calc
    def iv_port_cumret():
        weighted_returns = iv_factor_weigths().values.reshape(
            -1, 1
        ).T * raw_px().pct_change().fillna(0)
    ...
    def alloc_div():
        ...
        if input.blending_type() == "ew":
            weights = pd.DataFrame(
                10000 / weights.shape[0], index=weights.index, columns=["Allocation"]
            )
